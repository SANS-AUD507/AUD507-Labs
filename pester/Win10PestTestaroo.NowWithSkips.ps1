...
Describe 'Acceptance Testing for Win10 VM' {
  ...
  BeforeAll {
    ...
    ## TODO: What happens in Final vs. Appbase??
    ## I have no clue what is setup during the "Final" setup step so this can change to anything
    ## if a file from final is found, run all the tests - inversed since looking for appbase logic

    $SkipInAppbase = !(Test-Path -Path "C:\Tools\sslyze\sslyze.exe" -PathType Leaf)

    ## skips are pretty neat because you can skip at all testing levels. I utilize this to skip
    ## some individual tests and some test groups.
    
  }
    
  Context 'Folder Existence Testing' {

    It 'AUD507-Labs Directory Exists' -Skip:$SkipInAppbase {
  ...
  Context 'Software Existence Testing' {
    ...
    It 'Assessor-GUI.exe Exists' -Skip:$SkipInAppbase {
    ...
    It 'Burp Suite Professional Exists' -Skip:$SkipInAppbase {
    ...
    It 'Sslyze.exe Exists' -Skip:$SkipInAppbase {
  ...
  Context 'File Existence Testing' -Skip:$SkipInAppbase {
  ...
  Context 'File Integrity Testing' -Skip:$SkipInAppbase {
